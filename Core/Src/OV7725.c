...
u8 OV7725_Init(void)
{
    ...
    GPIO_InitStruct.Pin = RCLK_Pin|WEN_Pin|WRST_Pin|RRST_Pin|OE_Pin;
    ...
    HAL_GPIO_WritePin(GPIOB, WEN_Pin|WRST_Pin|RRST_Pin|OE_Pin, GPIO_PIN_RESET);
    ...
    HAL_Delay(50);
    reg=SCCB_RD_Reg_ecc(0X1c);		//读取厂家ID 高八位
    HAL_Delay(50);
    reg<<=8;
    reg|=SCCB_RD_Reg_ecc(0X1d);		//读取厂家ID 低八位
    if(reg!=OV7725_MID)
    {
        printf("Incorrect, MID:%d\n",reg);
//        return 1;
    }
    else
    {
        printf("Correct, MID:%d\n",reg);
    }
    reg=SCCB_RD_Reg_ecc(0X0a);		//读取厂家ID 高八位
    reg<<=8;
    reg|=SCCB_RD_Reg_ecc(0X0b);		//读取厂家ID 低八位
    if(reg!=OV7725_PID)
    {
        printf("Incorrect, HID:%d\n",reg);
//        return 2;
    }
    else
    {
        printf("Correct, HID:%d\n",reg);
    }
//    初始化 OV7725,采用QVGA分辨率(320*240)
    for(i=0;i<sizeof(ov7725_init_reg_tb1)/sizeof(ov7725_init_reg_tb1[0]);i++)
    {
        do {
            SCCB_WR_Reg_ecc(ov7725_init_reg_tb1[i][0],ov7725_init_reg_tb1[i][1]);
            HAL_Delay(5);
//            printf("%x, %x, %x\n", ov7725_init_reg_tb1[i][0], SCCB_RD_Reg_ecc(ov7725_init_reg_tb1[i][0]), ov7725_init_reg_tb1[i][1]);
        }while(SCCB_RD_Reg_ecc(ov7725_init_reg_tb1[i][0]) != ov7725_init_reg_tb1[i][1]);
    }
    printf("OV7725 Initialized\n");
    return 0x00; 	//ok
}
////////////////////////////////////////////////////////////////////////////
//OV7725功能设置
